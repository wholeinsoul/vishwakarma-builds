...
import { handleMessage, IncomingMessage } from '../conversation/engine.js';
import { WhatsAppClient, createWhatsAppClient } from '../services/whatsapp-client.js';
...
async function getImageUrl(imageId: string): Promise<string> {
  ...
  const response = await axios.get(
    `${config.whatsapp.apiBaseUrl}/${imageId}`,
    {
...
export async function registerWebhookRoutes(app: FastifyInstance, whatsappClient?: WhatsAppClient): Promise<void> {
  const sharedClient = whatsappClient || createWhatsAppClient();
  // Webhook verification (GET)
  app.get('/webhook/whatsapp', async (
    request: FastifyRequest<{ Querystring: WebhookQuerystring }>,
    reply: FastifyReply
  ) => {
  ...
  app.post('/webhook/whatsapp', async (
    request: FastifyRequest<{ Body: WebhookBody & { __rawBody?: Buffer } }>,
    reply: FastifyReply
  ) => {
