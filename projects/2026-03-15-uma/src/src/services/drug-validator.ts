...
let drugDatabase: DrugEntry[] | null = null;

function loadDrugDatabase(): DrugEntry[] {
  if (drugDatabase) return drugDatabase;
  const currentDir = dirname(fileURLToPath(import.meta.url));
  const dataPath = join(currentDir, '..', 'data', 'indian-drugs.json');
  const raw = readFileSync(dataPath, 'utf-8');
  drugDatabase = JSON.parse(raw) as DrugEntry[];
  return drugDatabase;
}
...
export function findDrug(drugName: string): DrugEntry | undefined {
  const db = loadDrugDatabase();
  const normalized = normalizeString(drugName);

  return db.find((drug) => {
    if (normalizeString(drug.generic_name) === normalized) return true;
...
export function validateMedication(
  ...
  const isCommonDose = drug.common_doses.some(
    (d) => d.toLowerCase() === normalizedDosage
  );
  if (!isCommonDose) {
    warnings.push(
      `Unusual dosage: ${dosage} for ${drug.generic_name}. Common doses: ${drug.common_doses.join(', ')}`
    );
  }
  ...
  if (doseMg && maxMg) {
    ...
    if (dailyTotal > maxMg) {
      warnings.push(
        `High dose warning: ${dosage} × ${timesPerDay}/day = ${dailyTotal}mg/day exceeds max ${drug.max_daily_dose} for ${drug.generic_name}`
      );
    }
  }
...
export function getDrugHindiName(drugName: string): string | undefined {
  const drug = findDrug(drugName);
  return drug?.hindi_name;
}

export function getDrugFoodInstructions(drugName: string): string | undefined {
  const drug = findDrug(drugName);
