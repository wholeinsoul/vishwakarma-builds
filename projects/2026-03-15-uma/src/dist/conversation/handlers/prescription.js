...
export async function handlePrescriptionImage(phone, imageUrl, context, whatsapp) {
    ...
    try {
        ...
        if (firstMed) {
            const confirmMsg = messages.confirmMedication(firstMed.drug_name_hi || firstMed.drug_name_en, firstMed.drug_name_en, firstMed.dosage || '', firstMed.frequency || '');
...
export async function handleMedicationConfirmation(phone, messageText, context, whatsapp) {
    ...
    const medIds = (context.medications_to_confirm || []);
    const currentIndex = (context.current_confirm_index || 0);
    ...
    if (isYes && currentMedId) {
        ...
        const skippedMsg = bilingual('❌ दवा हटा दी गई। अगली दवा देखते हैं।', '❌ Medication removed. Checking next one.');
    ...
    if (nextMed) {
        const confirmMsg = messages.confirmMedication(nextMed.drug_name_hi || nextMed.drug_name_en, nextMed.drug_name_en, nextMed.dosage || '', nextMed.frequency || '');
        await whatsapp.sendText(phone, confirmMsg);
    }
    return {
        newState: ConversationState.CONFIRMING_MEDICATIONS,
